...
### Personal features and runtime settings

With the local Docker provider, `--extra-devcontainer-path` lets you keep personal features and runtime settings outside the project repository:
...
```bash
devpod up . --extra-devcontainer-path ../personal-devcontainer/extra.json --recreate
```

Features from the extra file participate in image, Dockerfile, and Docker Compose builds. For the same feature ID, the extra file replaces the project's feature options, and `--additional-features` takes precedence over both. The entire option value is replaced, rather than merging individual options. Other feature IDs are preserved. Feature option values support the same variable substitution as the project's configuration, including `${localEnv:VARIABLE}`.

Use `--recreate` to apply feature changes to an existing workspace. A normal `up` on a running container does not install new features.
