...
var _ = ginkgo.Describe("devpod ssh test suite", ginkgo.Label("ssh"), ginkgo.Ordered, func() {
	...
	ginkgo.It(
		"should sign commits on first start when devcontainer has forwardPorts",
		ginkgo.SpecTimeout(7*time.Minute),
		func(ctx ginkgo.SpecContext) {
			if runtime.GOOS == osWindows {
				ginkgo.Skip("skipping on windows")
			...
			// This test reproduces the race condition from #645: when a devcontainer
			// has forwardPorts configured, the old setupGitSSHSignature function
			// would open an ephemeral SSH session whose port forwards conflicted
			// with the main tunnel, causing the signing helper setup to fail.
			tempDir, err := framework.CopyToTempDir("tests/ssh/testdata/ssh-signing-with-ports")
			framework.ExpectNoError(err)
			...
			// Generate a temporary SSH key for signing
			sshKeyDir, err := os.MkdirTemp("", "devpod-ssh-signing-ports-test")
			framework.ExpectNoError(err)
			defer func() { _ = os.RemoveAll(sshKeyDir) }()
			...
			// First start with signing key and forwarded ports — this must succeed
			// without the port forwarding race condition.
			err = f.DevPodUp(ctx, tempDir, "--git-ssh-signing-key", keyPath+".pub")
			framework.ExpectNoError(err)
			...
			gomega.Expect(strings.TrimSpace(out)).To(
				gomega.Equal("EXISTS"),
				"devpod-ssh-signature helper should be installed on first start with forwardPorts",
			)
			...
			// Attempt a signed commit through the tunnel
			commitCmd := strings.Join([]string{
				"cd /tmp",
				"git init test-sign-ports-repo",
				"cd test-sign-ports-repo",
				"git config user.name 'Test User'",
				"git config user.email 'test@example.com'",
				"git config commit.gpgsign true",
				"echo test > testfile",
				"git add testfile",
				"git commit -m 'signed commit with forwarded ports' 2>&1",
			}, " && ")
			...
			gomega.Expect(stdout).To(
				gomega.ContainSubstring("signed commit with forwarded ports"),
				"git commit should succeed on first start with forwardPorts configured",
			)
			...
			gomega.Expect(combined).To(
				gomega.ContainSubstring("Good"),
				"signature should be valid on first start with forwardPorts",
			)
		},
	)
