	...
	func() {
		...
		ginkgo.It("root folder", func(ctx context.Context) {
			tempDir, workspace, err := tc.setupAndStartWorkspace(
				ctx,
				"tests/up-docker-compose/testdata/docker-compose",
			)
			framework.ExpectNoError(err)
		...
		ginkgo.It("sub-folder", func(ctx context.Context) {
			tempDir, workspace, err := tc.setupAndStartWorkspace(
				ctx,
				"tests/up-docker-compose/testdata/docker-compose-subfolder",
			)
			framework.ExpectNoError(err)
			framework.ExpectNoError(tc.verifyWorkspaceMount(ctx, workspace, tempDir))
		}, ginkgo.SpecTimeout(framework.GetTimeout()))
		...
		ginkgo.It("overrides", func(ctx context.Context) {
			tempDir, workspace, err := tc.setupAndStartWorkspace(
				ctx,
				"tests/up-docker-compose/testdata/docker-compose-overrides",
			)
			framework.ExpectNoError(err)
			framework.ExpectNoError(tc.verifyWorkspaceMount(ctx, workspace, tempDir))
		}, ginkgo.SpecTimeout(framework.GetTimeout()))
		...
		ginkgo.It("env-file", func(ctx context.Context) {
			tempDir, err := setupWorkspace(
				"tests/up-docker-compose/testdata/docker-compose-env-file",
				tc.initialDir,
				tc.f,
			)
			framework.ExpectNoError(err)
		...
		ginkgo.It("restart", func(ctx context.Context) {
			...
			initialIds, err := findComposeContainer(
				ctx,
				tc.dockerHelper,
				tc.composeHelper,
				workspace.UID,
		...
		ginkgo.It("environment variables", func(ctx context.Context) {
			_, workspace, err := tc.setupAndStartWorkspace(
				ctx,
				"tests/up-docker-compose/testdata/docker-compose-container-env",
			)
			framework.ExpectNoError(err)
		...
		ginkgo.It("user", func(ctx context.Context) {
			_, workspace, err := tc.setupAndStartWorkspace(
				ctx,
				"tests/up-docker-compose/testdata/docker-compose-container-user",
			)
			framework.ExpectNoError(err)
		...
		ginkgo.It("override command", func(ctx context.Context) {
			_, workspace, err := tc.setupAndStartWorkspace(
				ctx,
				"tests/up-docker-compose/testdata/docker-compose-override-command",
			)
			framework.ExpectNoError(err)
		...
		ginkgo.It(
			...
			func(ctx context.Context) {
				...
				tempDir, err := setupWorkspace(
					"tests/up-docker-compose/testdata/docker-compose-uid-mapping",
					tc.initialDir,
					tc.f,
				)
				framework.ExpectNoError(err)
		...
		ginkgo.It(
			"implements updateRemoteUserUID with non-root container user (vscode)",
			func(ctx context.Context) {
				currentUser, err := user.Current()
				framework.ExpectNoError(err)
				...
				tempDir, err := setupWorkspace(
					"tests/up-docker-compose/testdata/docker-compose-uid-mapping-vscode",
					tc.initialDir,
					tc.f,
				)
				framework.ExpectNoError(err)
		...
		ginkgo.It("privileged", func(ctx context.Context) {
			_, workspace, err := tc.setupAndStartWorkspace(
				ctx,
				"tests/up-docker-compose/testdata/docker-compose-privileged",
			)
			framework.ExpectNoError(err)
		...
		ginkgo.It("capabilities", func(ctx context.Context) {
			_, workspace, err := tc.setupAndStartWorkspace(
				ctx,
				"tests/up-docker-compose/testdata/docker-compose-capadd",
			)
			framework.ExpectNoError(err)
			...
			gomega.Expect(detail.HostConfig.CapAdd).
				To(gomega.Or(gomega.ContainElement("SYS_PTRACE"), gomega.ContainElement("CAP_SYS_PTRACE")),
					"image capabilities are not duplicated")
			gomega.Expect(detail.HostConfig.CapAdd).
		...
		ginkgo.It("security options", func(ctx context.Context) {
			_, workspace, err := tc.setupAndStartWorkspace(
				ctx,
				"tests/up-docker-compose/testdata/docker-compose-securityOpt",
			)
			framework.ExpectNoError(err)
		...
		ginkgo.It("remote env", func(ctx context.Context) {
			_, workspace, err := tc.setupAndStartWorkspace(
				ctx,
				"tests/up-docker-compose/testdata/docker-compose-remote-env",
			)
			framework.ExpectNoError(err)
		...
		ginkgo.It("remote user", func(ctx context.Context) {
			_, workspace, err := tc.setupAndStartWorkspace(
				ctx,
				"tests/up-docker-compose/testdata/docker-compose-remote-user",
			)
			framework.ExpectNoError(err)
		...
		ginkgo.It("variables substitution", func(ctx context.Context) {
			tempDir, workspace, err := tc.setupAndStartWorkspace(
				ctx,
				"tests/up-docker-compose/testdata/docker-compose-variables",
			)
			framework.ExpectNoError(err)
			...
			localWorkspaceFolder, err := tc.execSSH(
				ctx,
				tempDir,
			...
			localWorkspaceFolderBasename, err := tc.execSSH(
				ctx,
				tempDir,
			...
			containerWorkspaceFolder, err := tc.execSSH(
				ctx,
				tempDir,
			...
			containerWorkspaceFolderBasename, err := tc.execSSH(
				ctx,
				tempDir,
