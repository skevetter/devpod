	...
	func() {
		...
		ginkgo.It(
			...
			func(ctx context.Context) {
				tempDir, err := setupWorkspace(
					"tests/up/testdata/docker-localworkspacefolder",
					initialDir,
					f,
				)
				framework.ExpectNoError(err)
		...
		ginkgo.It(
			"should rebuild image in case of changes in files in build context",
			func(ctx context.Context) {
				tempDir, err := setupWorkspace(
					"tests/up/testdata/docker-dockerfile-buildcontext",
					initialDir,
					f,
				)
				framework.ExpectNoError(err)
		...
		ginkgo.It(
			"should not rebuild image for changes in files mentioned in .dockerignore",
			func(ctx context.Context) {
				tempDir, err := setupWorkspace(
					"tests/up/testdata/docker-dockerfile-buildcontext",
					initialDir,
