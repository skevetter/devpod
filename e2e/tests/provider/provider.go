...
`
		...
		// RENAME-2.
		ginkgo.It("should fail to rename a provider to a name that already exists", func(ctx context.Context) {
			tempDir, err := framework.CopyToTempDir("tests/provider/testdata/simple-k8s-provider")
			framework.ExpectNoError(err)
			ginkgo.DeferCleanup(framework.CleanupTempDir, initialDir, tempDir)
		...
		// RENAME-4.
		ginkgo.It("should rename a provider with an associated stopped workspace", func(ctx context.Context) {
			f := framework.NewDefaultFramework(initialDir + "/bin")
		...
		// RENAME-5.
		ginkgo.It("should fail to rename a provider with a running workspace", func(ctx context.Context) {
			tempDir, err := framework.CopyToTempDir("tests/up/testdata/no-devcontainer")
			framework.ExpectNoError(err)
			ginkgo.DeferCleanup(framework.CleanupTempDir, initialDir, tempDir)
